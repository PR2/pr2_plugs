...
def execute_cb(goal):
  ...
  try:
    ...
    if joint_space_client.send_goal_and_wait(get_action_goal('move_arm_to_outlet'), rospy.Duration(20.0), preempt_timeout) != GoalStatus.SUCCEEDED:
    ...
    if joint_space_client.send_goal_and_wait(get_action_goal('move_arm_outlet_to_free'), rospy.Duration(20.0), preempt_timeout) != GoalStatus.SUCCEEDED:
    ...
    # Set aborted 
    server.set_aborted()
    rospy.loginfo("Failed to detect the outlet.")  
